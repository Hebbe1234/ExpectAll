...
case $EXPERIMENT in
	...
	1)
		...
		output=$(bash run_all.sh ../src ../src/topologies/japanese_topologies/ ../src/topologies/japanese.txt ../out/$outdir run_bdd.py single_path_limited_increasing 1 20 10 2 $BASHFILE);
		echo $output; #not necessary, just to see jobs we await
		sbatch --dependency=afterany:$output ./make_single_graph.sh $EXPERIMENT $outdir;; 
